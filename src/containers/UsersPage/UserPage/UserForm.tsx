...
import { addUser, getUser, removeUser, resetUser, saveUser, unblockUser } from '@apiServices/usersService';
...
import { getUserAppsCheckboxes, getUserAppsCheckboxesResult } from './UserFormHelper';
...
type userMessageParams = {
    ...
    pwd?: string;
    mode?: userMessageModes | string;
    errorMessage?: React.ReactNode;
};

type userMessageModes = 'CREATE' | 'EDIT' | 'RESTORED' | 'DELETE' | 'ERROR';

const userMessage = (
    login?: userMessageParams['login'],
    pwd?: userMessageParams['pwd'],
    mode?: userMessageParams['mode'] | string,
    errorMessage?: userMessageParams['errorMessage']
) => {
    switch (mode) {
        case MODE.CREATE: {
            return {
...
const showNotify = ({ login, pwd, mode, errorMessage }: userMessageParams) => {
...
const errorEditPermissions = (personalNumber: string) => (
    <span>
...
const UserForm: React.FC<UserFormProps> = ({ type, matchPath }) => {
    ...
    const getUserData = async () => {
        try {
            ...
            if (notNewUser) {
                ...
                setSalePoint({ id: user.salePointId, name: user.salePointName } as SalePointDto);
                setUserData(user);
    ...
    const onSubmit = async () => {
        ...
        try {
            const requestData = { clientAppIds, salePointId: salePoint?.id as number };
    ...
    const onSalePointChange = (salePoint: SalePointDto | null) => {
